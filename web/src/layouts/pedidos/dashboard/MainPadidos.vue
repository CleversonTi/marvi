...
const periodoFormatado = computed(() => {
 ...
 console.log(thisStartDate.value, thisStartEnd.value);
  return formatarPeriodo(thisStartDate.value, thisStartEnd.value);
});
console.log(periodoFormatado)
...
<template>
  ...
  <section class="primary_section">
    <div>
      <h1>Pedidos do Magento</h1>
      <div v-if="pedidosMg.carregando">
        🔄 Carregando...
      </div>
      <div v-else-if="pedidosMg.erro">
        {{ pedidosMg.erro }}
      </div>
      <ul v-else>
        <li
          v-for="pedido in pedidosMg.pedidos"
          :key="pedido.entity_id"
        >
          #{{ pedido.increment_id }} - {{ pedido.status }}
        </li>
      </ul>
